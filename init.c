  ...
  */ 
  ...
  if(nloops>=1){
	...
	rho_av = 0.25*(
		       ...
		       p[i][j-1][k][RHO] +
		       p[i-1][j-1][k][RHO]) ;
	aplus=aphiloop(r*sin(th), r*cos(th), rloop, zloop);
	if(evenloops) aplus-=aphiloop(r*sin(th), r*cos(th), rloop, -zloop); // suppose we want exactly zero flux through the eqplane
...
// smeared localized magnetic loop, see Abolmasov (2014) sec 5.1.2; based on the textbook electrodynamic problem from Batygin V. V., Toptygin I. N.
double aphiloop(double varpi, double z,  double varpi0, double z0)
{
  double kk=2.*sqrt(varpi0*varpi/((varpi+varpi0)*(varpi+varpi0)+(z-z0)*(z-z0)));
  double Kint=gsl_sf_ellint_Kcomp(kk, 0), Eint=gsl_sf_ellint_Ecomp(kk, 0);
  double d2=(varpi-varpi0)*(varpi-varpi0)+(z-z0)*(z-z0);
